"""Tests for domain data models: CalculationType, CalculationRequest, CalculationResult."""
...
def test_calculation_type_str_returns_name(
...
def test_calculation_request_stores_fields() -> None:
    ...
    request = CalculationRequest(
        operation=CalculationType.ADD, value1=3.5, value2=7.0
    )
    assert request.operation == CalculationType.ADD
    assert request.value1 == 3.5
    assert request.value2 == 7.0


def test_calculation_request_is_frozen() -> None:
    """CalculationRequest is immutable after construction."""
    request = CalculationRequest(
        operation=CalculationType.ADD, value1=1.0, value2=2.0
    )
...
def test_calculation_result_stores_result() -> None:
    ...
    result = CalculationResult(result=42.0)
