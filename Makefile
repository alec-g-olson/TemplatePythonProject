...
DOCKER_REMOTE_PULUMI = $(DOCKER_REMOTE_DEV_ROOT)/pulumi
DOCKER_REMOTE_SRC_AND_TEST = $(DOCKER_REMOTE_SRC) $(DOCKER_REMOTE_TEST)
DOCKER_REMOTE_DEV = $(DOCKER_REMOTE_SRC) $(DOCKER_REMOTE_TEST) $(DOCKER_REMOTE_BUILD_SUPPORT)
DOCKER_REMOTE_DOCUMENTED = $(DOCKER_REMOTE_SRC) $(DOCKER_REMOTE_BUILD_SUPPORT) $(DOCKER_REMOTE_PULUMI)
DOCKER_REMOTE_ALL_PYTHON_FOLDERS = $(DOCKER_REMOTE_DOCUMENTED) $(DOCKER_REMOTE_TEST)
...
.PHONY: test
test: test_without_style # build_pulumi_environment
	...
	$(DOCKER_COMMAND) pydocstyle $(DOCKER_REMOTE_DOCUMENTED)
	$(DOCKER_COMMAND) pydocstyle --add-ignore=D100,D104 $(DOCKER_REMOTE_TEST)
	$(DOCKER_COMMAND) flake8 $(DOCKER_REMOTE_ALL_PYTHON_FOLDERS)
	$(DOCKER_COMMAND) mypy $(DOCKER_REMOTE_DEV)
	# $(PULUMI_DOCKER_COMMAND) mypy $(DOCKER_REMOTE_PULUMI)
