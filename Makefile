...
test: test_without_style
	$(DOCKER_COMMAND) isort --check-only $(DOCKER_REMOTE_SRC_AND_TEST)
	$(DOCKER_COMMAND) black --check $(DOCKER_REMOTE_SRC_AND_TEST)
	$(DOCKER_COMMAND) pydocstyle $(DOCKER_REMOTE_SRC)
	$(DOCKER_COMMAND) pydocstyle --add-ignore=D100,D104 $(DOCKER_REMOTE_TEST)
	$(DOCKER_COMMAND) flake8 $(DOCKER_REMOTE_SRC_AND_TEST)
	$(DOCKER_COMMAND) mypy $(DOCKER_REMOTE_SRC_AND_TEST)
...
lint: build_dev_environment
	$(DOCKER_COMMAND) isort $(DOCKER_REMOTE_SRC_AND_TEST)
	$(DOCKER_COMMAND) black $(DOCKER_REMOTE_SRC_AND_TEST)
