...
def _get_github_license_template_info_blobs() -> list[dict[str, str]]:
    ...
    return json.loads(requests.get(url=GIT_HUB_TEMPLATE_URL).text)
...
def get_template_for_license(template_key: str) -> str:
    ...
    assert template_key_lower in get_licenses_with_templates()
    if template_key_lower == ALL_RIGHTS_RESERVED_KEY:
        ...
        template = json.loads(requests.get(url=template_info["url"]).text)
